...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        //insert ignore 如果重复插入同样的项，就相当于不处理
    ...
    //因为实现Insert a favorite record to the database，需要item先存在，所以要先save一下
    public void saveItem(Item item) throws MySQLException {
        ...
        try {
            ...
            statement.setString(7, item.getType().toString());//enum要toString，db才能认识
            statement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            ...
            //利用循环，rs.next()每次读取一行数据。在每一行数据中只提取item_id这个column
            while (rs.next()) {
    ...
    public Map<String, List<String>> getFavoriteGameIds(Set<String> favoriteItemIds) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            //相当于用java程序实现join（）功能，join（）同时对2个表进行query
            for (String itemId : favoriteItemIds) {
                statement.setString(1, itemId);
                ResultSet rs = statement.executeQuery();
                if (rs.next()) {
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        //为了简单，假设可以重复注册同一个人（允许同一个id重复插入）（IGNORE），其实如果数据库有这个人了应该返回fail
